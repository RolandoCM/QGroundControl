...
void QGCHilConfiguration::on_simComboBox_currentIndexChanged(int index)
{
    ...
    {
        // Ensure the sim exists and is disabled
        _vehicle->uas()->enableHilCondor(false);
        QGCHilCondorConfiguration* hxpconf = new QGCHilCondorConfiguration(_vehicle->uas()->getHILSimulation(), this);
        hxpconf->show();
        ui->simulatorConfigurationLayout->addWidget(hxpconf);

        // Select correct version of condor
        QGCCondorLink* condor = dynamic_cast<QGCCondorLink*>(_vehicle->uas()->getHILSimulation());
        if (condor)
        {
           // condor->setVersion((index == 2) ? 10 : 9);
            connect(condor, &QGCCondorLink::statusMessage, ui->statusLabel, &QLabel::setText);
        }
    }
