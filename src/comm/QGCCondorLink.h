...
/**
 * @file QGCCondorLink.h
 *   @brief Condor simulation link
 *
 *
 */

#ifndef QGCCONDORSIMULATIONLINK_H
#define QGCCONDORSIMULATIONLINK_H
...
class QGCCondorLink : public QGCHilLink
{
    Q_OBJECT
    //Q_INTERFACES(QGCCondorLinkInterface:LinkInterface)

public:
    QGCCondorLink(Vehicle* vehicle, QString remoteHost=QString("127.0.0.1:55286"), QHostAddress localHost = QHostAddress::Any, quint16 localPort = 55279);
    ~QGCCondorLink();

    /**
     * @brief Load Condor HIL settings
     */
    void loadSettings();

    /**
     * @brief Store Condor HIL settings
     */
    void storeSettings();
    ...
    QString getVersion()
    {
        return QString("Condor %1").arg(condorVersion);
    }
...
protected:
    ...
    float vx, vy, vz, xacc, yacc, zacc, axC, ayC, azC;
    float ind_airspeed;
    float true_airspeed;
    float groundspeed;
    float yawmag, xmag, ymag, zmag, abs_pressure, diff_pressure, pressure_alt, temperature;
    float barometerOffsetkPa;
    float vario, evario, nettovario;

    float man_roll, man_pitch, man_yaw;
    QString airframeName;
    enum AIRFRAME airframeID;
    bool condorConnected;
    unsigned int condorVersion;
...
#endif // QGCCONDORSIMULATIONLINK_H
