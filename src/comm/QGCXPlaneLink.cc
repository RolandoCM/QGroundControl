...
void QGCXPlaneLink::run()
{
    ...
    //update controls para envio descartada
...
void QGCXPlaneLink::readBytes()
{
    ...
    qDebug()<<data;
    //qDebug()<<"ip server: "<<sender.toString();
    //qDebug()<<"port server: "<<senderPort;
            ...
            data[3] == 'A')
    {
        ...
        for (unsigned i = 0; i < nsegs; i++)
        {
           ...
           //         qDebug() << "X-Plane values" << " xacc "<< xacc <<" yacc "<< yacc << "zacc" <<zacc;
				}
         ...
         //       qDebug()<<"dataset 6";
       //         qDebug()<<"Excecute... 6     " << fields_changed<<"  abs: "<<abs_pressure;
            }
...
//            }
            else if ((xPlaneVersion == 10 && p.index == 16) || (xPlaneVersion == 9 && p.index == 17))
            {
                ...
                rollspeed = p.f[1];
                yawspeed = p.f[2];
                //qDebug()<<"pitchspeed: "<<pitchspeed<< " in radians "<<pitchspeed/180.0f*M_PI;
                //qDebug()<<"yawspeed: "<<yawspeed<< " in radians "<<yawspeed/180.0f*M_PI;
                //qDebug()<<"rollspeed: "<<rollspeed<< " in radians "<<rollspeed*180.0f/M_PI;
            ...
            else if ((xPlaneVersion == 10 && p.index == 17) || (xPlaneVersion == 9 && p.index == 18))
            {
               ...
               // qDebug()<<"Excecute... pitch" << fields_changed;
    ...
    if (emitUpdate && (QGC::groundTimeMilliseconds() - simUpdateLast) > 2)
    {
        ...
        if (_sensorHilEnabled)
        {
            ...
            emit hilStateChanged(QGC::groundTimeUsecs(), roll, pitch, yaw, rollspeed,
                                 pitchspeed, yawspeed, 0.0, lat, lon, alt,
...
void QGCXPlaneLink::setPositionAttitude(double lat, double lon, double alt, double roll, double pitch, double yaw)
{
    ...
    // qDebug()<<"Pitch in position  --- " << pitch;
