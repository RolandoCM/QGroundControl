...
void UAS::receiveMessage(mavlink_message_t message)
{
    ...
    if (message.sysid == uasId && (!attitudeStamped || (attitudeStamped && (lastAttitude != 0)) || message.msgid == MAVLINK_MSG_ID_ATTITUDE))
    {
        ...
        case MAVLINK_MSG_ID_HIL_CONTROLS:
        {
            ...
            // qDebug()<<"esto es hil"<<hil.time_usec<<hil.roll_ailerons; //#########################################################
...
//condor simulator #################################################################################
void UAS::enableHilCondor(bool enable)
{
   // qDebug()<<"USE enableHilCondor "; // ########################################################
    QGCCondorLink* link = dynamic_cast<QGCCondorLink*>(simulation);
    if (!link) {
        if (simulation) {
            stopHil();
            delete simulation;
        }
        simulation = new QGCCondorLink(_vehicle);
...
void UAS::sendHilState(quint64 time_us, float roll, float pitch, float yaw, float rollspeed,
                       float pitchspeed, float yawspeed,float vario, double lat, double lon, double alt,
                       float vx, float vy, float vz, float ind_airspeed, float true_airspeed, float xacc, float yacc, float zacc)
{
    //qDebug()<<"Use sendHilState"; //#####################################################################################
    //qDebug()<<"variometer: "<< vario;
    if (!_vehicle) {
        return;
    }
    ...
    if (_vehicle->hilMode())
    {
        ...
        mavlink_msg_hil_state_quaternion_pack_chan(mavlink->getSystemId(),
                                                   ...
                                                   _vehicle->priorityLink()->mavlinkChannel(),
                                                   &msg,
                                                   time_us, q, rollspeed, pitchspeed, yawspeed, vario,
...
{
    qDebug()<<"USe sendHilOpticalFlow"; //####################################################################################3
    if (!_vehicle) {
        return;
    }
